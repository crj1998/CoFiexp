...
class EvolutionSearcher:
    ...
    def random(self, population):
        while len(population) < self.population_num:
            config = self.resample()
            self._cache[config] = self.metric_func(config)
    ...
    def mutation(self, parents):
        ...
        while len(res) < self.mutation_num:
            for _ in range(self.MAX_RETRY):
                ...
                config = Individual({
                    k: max(min(v + (random.choice(self.search_space[k]) if random.random() < self.mutation_prob else 0), 3072), 1)
    ...
    def run(self, init=None):
        ...
        for _ in range(1, self.evolution_iters):
            parents = sorted(population, key=lambda x: self._cache[x], reverse=(self.optimize == 'max'))[:self.parent_num]
            mutation = self.mutation(parents)
            crossover = self.crossover(parents)
            population = population + mutation + crossover
            self.random(population)
            population = sorted(population, key=lambda x: self._cache[x], reverse=(self.optimize == 'max'))[:self.population_num]
        print({p: self._cache[p] for p in population[:20]})


if __name__ == '__main__':

    # item = Individual({"hidden": 4, "layer": 8, "head": 6, "intermediate": 16})
    # print(hash(item))
    search_space = {
        f"layer_{layer}": [-4, -2, -1, 1, 2, 4] for layer in range(12)
    }
    def f(**kwargs):
        return kwargs["layer_1"] + kwargs["layer_3"]
