...
class L0Module(nn.Module):
    ...
    def get_num_parameters_and_constraint(self, hidden=False):
        ...
        # 12 * 1 * 1
        mha_score = self.score_loga(self.mha_loga).view(-1, 1, 1) if "mha" in self.types else None
        # 12 * 12 * 1
        heads_score = self.score_loga(self.heads_loga).unsqueeze(dim=-1)

        if hidden:
            hidden_score = self.score_loga(self.hidden_loga)
            heads_score = (heads_score * mha_score) if mha_score is not None else heads_score
            ...
            num_parameters += heads_score.sum() * self.parameters_per_dim["head"]

        # 12 * 1
        ffn_score = self.score_loga(self.ffn_loga).unsqueeze(dim=-1)
        # 12 * 3072
        intermediate_score = self.score_loga(self.intermediate_loga)
    ...
    def calculate_model_size(self, zs):
        ...
        mha_z = numpified_zs["mha"].reshape(-1, 1)
